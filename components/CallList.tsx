        ...
        const fetchRecordings = async () => {
            try {
                const callData = await Promise.all(callRecordings.map(async (meeting) => {
                    ...
                    return result || { recordings: [] };  // Ensure we always return an object with a 'recordings' property
                }))
                ...
                toast({ title: "Try again later", type: "error" })
            }
        }
        ...
        <div className='grid grid-cols-1 gap-5 xl:grid-cols-2'>
            {calls && calls.length > 0 ? calls.map((meeting: Call | CallRecording) => (
                <MeetingCard
                    ...
                    title={(meeting as Call).state?.custom.description.substring(0, 26) || (meeting as CallRecording).filename.substring(0, 20) || "No Description Available"}
